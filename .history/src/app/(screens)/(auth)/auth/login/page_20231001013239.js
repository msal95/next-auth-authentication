  ...
  const handleSubmit = async () => {
    axios
      ...
      .then((res) => {
        ...
        console.log("🚀 ~ file: page.js:22 ~ .then ~ response:", response);

        if (response.status === 200) {
          console.log(
            "🚀 ~ file: page.js:21 ~ axios.post ~ response:",
            response?.message
          );
          signIn("credentials", {
            email: authState.email,
            password: authState.password,
            callbackUrl: response?.datat?.role === "Admin" ? "/dashboard" : "/",
            redirect: true,
          });
          // router.push("/auth/login");
        } else {
          console.log(
            "🚀 ~ file: page.js:24 ~ axios.post ~ response.error:",
            response.errors
          );
        }
      })
      .catch((error) => {
        console.log("🚀 ~ file: page.js:27 ~ axios.post ~ error:", error);
      });
  };
